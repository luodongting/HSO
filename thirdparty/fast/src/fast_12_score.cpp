...
inline int fast_corner_score_12(const fast_byte* cache_0, const int offset[], int b)
{	
	b++;
  ...
  std::vector<int>& scores)
{
  ...
  for(unsigned int n=0; n < corners.size(); n++)
    scores[n] = fast_corner_score_12(img + corners[n].y*img_stride + corners[n].x, pixel, threshold);
}
