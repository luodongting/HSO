...
namespace fast
{
void fast_corner_detect_plain_8(const fast_byte* img, int img_width, int img_height, int img_stride,
                           		short barrier, std::vector<fast_xy>& corners)
{
	...
	for(y = 3 ; y < img_height - 3; y++)
	{
		...
		for(; cache_0 < line_max;cache_0++)
		{
			...
			success:
				corners.push_back(fast_xy(static_cast<short>(cache_0-line_min), static_cast<short>(y)));
		}
	}
}
}
